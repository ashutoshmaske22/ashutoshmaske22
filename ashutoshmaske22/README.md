...
Manual UI clicks to set up OWUI resources don't scale. This wrapper makes it reproducible and version-controlled.

**What it does:**
- `createOrUpdateGroup` · `createOrUpdateKnowledgeBase` · `createOrUpdateModel`
- `bootstrap()` — one call creates group + KB + model with access control wired automatically
- Zero external dependencies — pure `urllib`
- Built and published via GitLab CI as a proper Python package (`.whl` + `.tar.gz`)
